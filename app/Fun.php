...
static public function noticia_query(Request $request, Response $response, $params) {
	$slug= $params['slug'];
    $db = new db();
    /// ANTIGA (problema idioma): $db->sql("select * from pagina,idioma where registreid=pagina.id and camp='slug' and  text='".$slug."' and pagina.tipus = 'N';");
    /// MOLT LENT: $db->sql("select * from _noticia where slug='".$slug."';");
	$db->sql("select ordre, iii.idioma as idioma, p.id, p.tipus,destacada,categoria,tags,url,json,alta,modificacio,publicacio,baixa,slug,
(select i.text from idioma i where i.registreid = p.id and i.idioma = iii.idioma and i.camp = 'titol') AS `titol`,
(select ii.text from idioma ii where ii.registreid = `p`.`id` and ii.idioma = iii.idioma and ii.camp = 'contingut') AS `contingut`
 from `idioma` iii,pagina p where iii.`registreid` = `p`.`id` and iii.`camp` = 'slug' and text='".$slug."';");
    $data = $db->all();
	if (empty($data)) :
		header("HTTP/1.0 404 Not found");
		die('{"error":"Noticia no trobada"}');
	endif;
	Fun::render($data);
}
...
static public function set_dataactiu(Request $request, Response $response, $params) {
	...
	$db->sql("update jugador set dataactiu='".date('Y')."1231235959' where numsoci in ('".implode("','",$jugadors)."');");
	return true;
}
